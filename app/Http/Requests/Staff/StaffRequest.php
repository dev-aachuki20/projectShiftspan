    ...
    public function rules(): array
    {
        ...
        if (!empty($ids)) {
            ...
            if(!empty($method == 'PUT')){
                ...
                $rules['name']                  = ['required', 'regex:/^[a-zA-Z\s]+$/','string', 'max:255', 'unique:users,name,'. $this->staff.',uuid,deleted_at,NULL', new NoMultipleSpacesRule];
                $rules['email']                 = ['required', 'email', 'regex:/(.+)@(.+)\.(.+)/i', 'unique:users,email,'. $this->staff.',uuid,deleted_at,NULL', new NoMultipleSpacesRule];
                ...
                $rules['title']                 = ['required', 'regex:/^[a-zA-Z\s]+$/','string', 'max:10', new NoMultipleSpacesRule];
                $rules['name']                  = ['required', 'regex:/^[a-zA-Z\s]+$/','string', 'max:255', new NoMultipleSpacesRule, Rule::unique('users')->ignore($this->input('id'), 'uuid')->whereNull('deleted_at')];
